...
export default async function FounderHQPage() {
  ...
  const [projects, statusCounts, milestones, activity, priorities] = await Promise.all([
  ...
  const atRiskOrOffTrack = projects.filter((p) => p.health !== 'on_track').length;
          ...
          <CardContent className="flex flex-col gap-2">
            {projects.length === 0 ? (
              ...
              projects.map((project) => (
                <div key={project.id} className="flex items-center justify-between rounded-md border border-border px-3 py-2">
                  <div>
